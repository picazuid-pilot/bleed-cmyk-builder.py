...
def create_bleed_image(original_img, bleed_pixels, method, custom_color=None):
    ...
    if method == "Wit / Geselecteerde Kleur":
        ...
        # FIX: We maken het canvas nu NIET leeg/zwart, maar we stretchen de originele afbeelding 
        # eerst over het gehele canvas als een 'veiligheidsvulling'. Dit maskeert subpixel-kieren volledig.
        new_img = original_img.resize((new_width, new_height), Image.Resampling.NEAREST)
        
        # Plak de echte originele afbeelding in het centrum
        new_img.paste(original_img, (bleed_pixels, bleed_pixels))
        
        # Mirror top edge (we pakken 1 pixel extra overlap om afrondingsfouten te doden)
        ...
        # Tot slot plakken we nogmaals het origineel vlijmscherp in het midden om eventuele 1-pixel overlaps op de basisfoto te herstellen
        new_img.paste(original_img, (bleed_pixels, bleed_pixels))
